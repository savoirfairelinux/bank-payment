...
class BankAccRecStatement(osv.osv):
    ...
    def unlink(self, cr, uid, ids, context=None):
        """ Reset the related account.move.line to be re-assigned
         later to statement."""
        ...
        for statement in self.browse(cr, uid, ids, context=context):
            ...
            statement_line_obj.unlink(cr, uid,
                                      statement_line_ids,
                                      context=context)
        return super(BankAccRecStatement, self).unlink(cr, uid, ids,
                                                       context=context)
    ...
    def check_difference_balance(self, cr, uid, ids, context=None):
        ...
        for stmt in self.browse(cr, uid, ids, context=context):
            if stmt.difference != 0.0 and not stmt.multi_currency:
                ...
                stmt.multi_currency and stmt.general_ledger_balance != \
                    stmt.registered_balance:
                raise orm.except_orm(
                    _('Warning'),
                    _('Prior to reconciling a statement in a foreign currency'
    ...
    def action_process(self, cr, uid, ids, context=None):
        ...
        Assign 'Bank Acc Rec Statement ID' for the statement lines which
    ...
    def action_cancel_draft(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context=context):
            ...
            account_move_line_obj.write(cr, uid,
                                        line_ids,
    ...
    def _get_balance(self, cr, uid, ids, field_names, args, context=None):
        ...
        Difference = (
        Ending Balance – Beginning Balance) - cleared balance = should be zero.
        """
        res = {}
    ...
    def refresh_record(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            ...
            line_ids = account_move_line_obj.search(cr, uid,
                                                    domain,
                                                    context=context)
    ...
    def is_b_a_r_s_state_done(self, cr, uid, move_line_id, context=None):
        ...
        for state_line in statement_line_ids:
            b_a_r_s_line = statement_line_obj.browse(
                ...
                state_line,
                context=context)
            b_a_r_s = self.browse(cr, uid,
                                  b_a_r_s_line.statement_id.id,
                                  context=context)
            if b_a_r_s and b_a_r_s.state not in ("done", "cancel"):
                return False
        return True

    def _get_last_reconciliation(self, cr, uid,
                                 account_id,
                                 context=None):
    ...
    def onchange_account_id(self, cr, uid, ids, account_id, ending_date,
        ...
        if account_id:
            ...
            # 1. credit and debit side journal items in posted state of the
            # selected GL account
            # 2. Journal items which are not assigned to previous bank
            # statements
    ...
    def _get_currency_help_label(self, cr, uid, currency_id, payment_rate,
        ...
        :return: translated string giving a tip on what's the effect
            of the current payment rate specified
        :rtype: str
        """
        rml_parser = report_sxw.rml_parse(cr, uid,
                                          'currency_help_label',
                                          context=context)
    ...
    def onchange_currency_rate(self, cr, uid, ids,
                               exchange_rate,
                               start_balance,
                               end_balance,
                               context=None):
    ...
    def onchange_exchange_date(self, cr, uid, ids,
                               exchange_date,
                               account_id,
                               context=None):
    ...
    _columns = {
        ...
        'sum_of_credits_lines': fields.function(
            ...
            method=True,
            type='float',
            string='Checks, Withdrawals, Debits, and Service Charges # of '
                   'Items',
            help="Total of number of lines with Cleared = True",
            multi="balance"),
        ...
        'suppress_ending_date_filter': fields.boolean(
            ...
            help="If this is checked then the Statement End Date filter"
                 " on the transactions below will not occur."
...
class BankAccRecStatementLine(osv.osv):
    ...
    _columns = {
        ...
        'research_required': fields.boolean(
            'Research Required? ',
            help='Check if the transaction should be researched by'
                 ' Accounting personal'),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not vals.get('move_line_id', False):
            raise orm.except_orm(
                _('Processing Error'),
                _('You cannot add any new bank statement line manually'
                  ' as of this revision!'))
        account_move_line_obj.write(
            cr, uid,
            [vals['move_line_id']],
